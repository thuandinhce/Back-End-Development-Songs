...
def create_song():
    song = request.json
    check_id = db.songs.find_one({"id": song["id"]})
...
def update_song(id):
    song = request.json
    check_id = db.songs.find_one({"id": id})
    if check_id:
        results = db.songs.update_one({"id": id}, {"$set": song})
...
######################################################################
# Implement the DELETE /song endpoint
######################################################################
